...
export const Cart = () => {
  ...
  const totalPrice = cart.reduce((sum:number,obj:MainItemProps) =>{
    return obj.price * obj.count + sum
},0)

  const discount = cart.reduce((sum,obj:MainItemProps) =>{
    return obj.discount * obj.count + sum
},0)

  const onClickClearCart = async () => {
    setDoc(doc(db, 'userCartItems', currentUser!.uid), {
            ...
            <div className="content__items">
              <div className="card">
                ...
                <div className="card__discount">
                  <div>Итоговая скидка</div>
                  <div>
                    <span> -{discount} $</span>
                  </div>
                </div>
                <div className="card__offerprice">
                  <div>К оплате</div>
                  <div className="price">{totalPrice - discount}$</div>
                </div>
                <Link to="/">
                  <button className="card__button">Оплатить заказ</button>
                </Link>
              </div>
              {cart.map((e: MainItemProps, i: number) => e.count > 0 && (
                <CartItem {...e} key={i} />
              ))}
            </div>
